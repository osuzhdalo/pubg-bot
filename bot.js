...
function getRankRoleName(tier, subTier) { 
  ...
  // Якщо це Master або Grandmaster, subTier часто немає — повертаємо просто назву рангу
  if (formatted.toLowerCase() === 'master' || formatted.toLowerCase() === 'grandmaster') {
    return formatted;
  }
...
async function updatePlayerStatsAndRoles(member, nickname) {
  ...
  // Регістронезалежне очищення старих ролей
  ...
  if (tier && tier !== "UNRANKED" && rankRoleName) {
    rolesToGiveNames.push(rankRoleName);
  }
  if (fppGames > 0) { 
    rolesToGiveNames.push(getFppAdrRole(fppAdr)); 
    const kdRole = getFppKdRole(fppKd);
    if (kdRole) rolesToGiveNames.push(kdRole); 
  }
  if (rankedGames > 0) { 
  ...
  for (const rName of rolesToGiveNames) {
    if (!rName) continue;
    // Регістронезалежний пошук ролі на сервері (працює і для Master, і для Platinum 1 тощо)
  ...
  const regRole = guild.roles.cache.find(r => r.name.toLowerCase() === "registered");
